...
buttonEls.forEach(button => button.addEventListener('click', 
    /**
    ...
    function clickHandler(event) {
        ...
        let currCategory = (event.target.dataset.type);
        let markup = showCategory(currCategory);
...
function showCategory(category) {
    let currProduct = products[`${category}`]
    let markup = ''
    for (let prod of currProduct){
        markup += getProductMarkup(prod);
